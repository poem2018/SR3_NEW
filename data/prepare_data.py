...
def resize_vlr(path, sizes, lmdb_save):
    print("start")
    ...
    for image_file in tqdm(image_files, desc="Loading images"):
        ...
        img_idx = image_path.split("/")[-1][:-6]
        img = img.convert('RGB')
        out = resize_multiple( img, sizes=sizes, resample=Image.BICUBIC, lmdb_save=False)
        lr_img, hr_img, sr_img = out
        sr_img.save('./dataset/test_Celeba_8x/sr_{}_{}/{}.png'.format(sizes[0], sizes[1], img_idx))

    return True



# def esr_downsampldata):
...
if __name__ == '__main__':
    print("starttttt")
    resize_vlr('./dataset/Celeba_8x_128_1024_16/lr_64/', sizes = (16, 128), lmdb_save=True) 
...
#     parser.add_argument('--lmdb', '-l', action='store_false')   ##store_true
